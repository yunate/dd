...
namespace NSP_DD {
class my_https_service;

static std::map<my_https_service*, std::shared_ptr<my_https_service>> g_services;
static bool g_stop = false;
static ddtask_thread_pool g_thread_pool(8);

static std::atomic_int count = 0;
class my_https_service : public ddhttp_service
{
public:
    my_https_service(std::shared_ptr<ddsocket_async> socket) :
...
DDTEST(test_case_https_server, tcp_server)
{
    ...
    auto server = ddtcp_server::create_instance(*iocp, [](std::shared_ptr<ddsocket_async> socket) {
        ...
        std::shared_ptr<ddcert> cert = ddcert::create_instance(LR"(F:\My\ddm\test\src\res\cret\ddm.pfx)", L"psw");
        std::unique_ptr<ddtls_server> tls_server(new ddtls_server(cert));
        std::shared_ptr<ddtls_socket_async> tls_socket = ddtls_socket_async::create_instance(std::move(tls_server), socket);
        tls_socket->hand_shake([tls_socket](bool successful) {
            if (!successful) {
                return;
            }
            auto service = new my_https_service(tls_socket);
            g_services[service] = std::shared_ptr<my_https_service>(service);
            service->start_recv();
        });
    });
