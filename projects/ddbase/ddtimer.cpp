...
u64 ddtime::get_experid_timestamp(u64 timeout)
{
    if (timeout == INFINITE) {
        return INFINITE;
    }

    return ddtime::now_ms() + timeout;
}

u64 ddtime::get_timeout(u64 experid_timestamp)
{
    if (experid_timestamp == INFINITE) {
        return INFINITE;
    }
    u64 now = ddtime::now_ms();
    if (now >= experid_timestamp) {
        return 0;
    }
    return experid_timestamp - now;
}
