...
{
public:
    static std::unique_ptr<ddtls> create_client(const std::string& server_name);

public:
    ddtls();
    virtual ~ddtls();

    using pfn_io_opt = std::function<void(void* buff, s32 buff_size, const std::function<void(bool successful, s32 byte)>& callback, ddexpire expire)>;
    void handshake(pfn_io_opt read_opt, pfn_io_opt write_opt, const std::function<void(bool successful)>& callback, const ddasync_caller& async_caller, ddexpire expire = ddexpire::never);
...
protected:
    ...
    void on_handshake_end(bool successful, const std::function<void(bool successful)>& callback);
...
{
    friend ddtls;
    ddtls_client(const std::string& server_name);
