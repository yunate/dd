...
public:
    ...
    // 枚举系统句柄表中可以解析名称的所有句柄
    // 不包含没有保留进程句柄的EXE/DLL映像和内存映射文件
    // 有极低的可能性crash, 这个crash无法避免, 也没有更好的解决方案, 因此建议在子进程中调用该函数
    ...
    // 只枚举系统句柄表中的File类型句柄, 例如普通打开的文件、目录、管道和设备
    // 不包含仅以映像或内存映射方式使用且没有保留File句柄的文件
    ...
    // 枚举进程地址空间中的映像和映射文件
    // type_name为Image时表示EXE/DLL, 为MappedFile时表示其他内存映射文件
    static bool enum_mapped_files(
        const std::function<void(const ddhandle_info&)>& pred,
        s32 thread_count = 32
    );

    // 枚举File类型句柄以及进程地址空间中的映像和映射文件
    static bool enum_file_usages(
        const std::function<void(const ddhandle_info&)>& pred,
        s32 thread_count = 32
    );
public:
