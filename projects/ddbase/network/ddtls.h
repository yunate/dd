...
public:
    ...
    // 该类有的操作比较耗时, 可以设置async_caller来让其在其他线程中执行
    void set_aync_caller(const ddasync_caller& async_caller);
    ...
    ddcoroutine<codec_result> encode(u8* buff, s32 buff_size);
    ...
    ddcoroutine<codec_result> decode(u8* buff, s32 buff_size);
...
protected:
    ...
    // weak
    std::weak_ptr<ddtls> m_weak_this;
};
