...
{
    m_parser.reset();
    m_recv_raw_buff.resize(0);
}

// client create tls
template <>
bool ddhttp_socket<ddhttp_request_header, ddhttp_response_header>::init_tls(const std::string& host)
{
    if (m_tls != nullptr) {
        return true;
    }
    ...
    m_tls->set_aync_caller(m_async_caller);
    return true;
}

template <>
bool ddhttp_socket<ddhttp_request_header, ddhttp_response_header>::init_tls(const std::shared_ptr<ddcert>&)
{
    DDASSERT_FMTW(false, L"The client do not support this function.");
    return false;
}

// server create tls
template <>
bool ddhttp_socket<ddhttp_response_header, ddhttp_request_header>::init_tls(const std::shared_ptr<ddcert>& cert)
{
    if (m_tls != nullptr) {
        return true;
    }
...
template <>
bool ddhttp_socket<ddhttp_response_header, ddhttp_request_header>::init_tls(const std::string&)
{
    DDASSERT_FMTW(false, L"The server do not support this function.");
    return false;
}
...
ddcoroutine<std::tuple<bool, s32>> DDHTTP_SOCKET::send_inner(void* buff, s32 buff_size, ddexpire expire)
{
    ...
    if (m_tls != nullptr) {
        auto encode_result = co_await m_tls->encode((u8*)buff, buff_size);
...
ddcoroutine<std::tuple<bool, s32>> DDHTTP_SOCKET::recv_inner(void* buff, s32 buff_size, ddexpire expire)
{
    if (m_tls != nullptr) {
        while (true) {
            ...
            auto decode_result = co_await m_tls->decode((u8*)buff, byte);
            if (!decode_result.successful) {
                co_return{ false, 0 };
            }

            push_back(m_recv_raw_buff, (u8*)decode_result.buff, decode_result.buff_size);
        }
    } else {
