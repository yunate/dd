...
public:
    ...
    */
    static ddfile* create_or_open(const std::wstring& path);
    ...
    */
    static ddfile* create_file_with_head_checker(const std::wstring& path, const u8* head_checker, s32 head_checker_size);

    // 创建一个utf16 le 编码的文件 使用read_linew读取文件
    // 文件soffset为2;
    static ddfile* create_usc2_file(const std::wstring& path);

    // 创建一个utf16 be  编码的文件 使用read_linew读取文件
    static ddfile* create_usc2_file_be(const std::wstring& path);

    // 创建一个UTF8 编码的文件 使用read_linea读取文件
    // 文件soffset为0;
    static ddfile* create_utf8_file(const std::wstring& path);

    // 创建一个UTF8bom 编码的文件 使用read_linea读取文件
    // 文件soffset为3;
    static ddfile* create_utf8bom_file(const std::wstring& path);
