...
DDTEST(test_ddjson, ddjson_value)
{
    // test copy
    {
        ddjson_value jv1;
        ddjson_value jv2(ddjvt::str);
        jv2.as_str() = "a";
        jv1 = jv2;
        DDASSERT(jv1.is_str());
        DDASSERT(jv1.as_str() == "a");
        DDASSERT(jv2.is_str());
        DDASSERT(jv2.as_str() == "a");
    }
...
DDTEST(ddjson_parse_from_file, ddjson_parse1)
{
    {
        std::unique_ptr<ddfile> file(ddfile::create_utf8_file(LR"(E:\My\workspaces\C++_workspaces\ddtest\json\bin\data\glossary.json)"));
        std::string json_str(file->file_size(), 0);
        file->read((u8*)json_str.data(), (s32)json_str.size());
        auto result = ddjson::parse(json_str);
    }
    std::vector<std::wstring> json_paths;
    dddir::enum_dir(LR"(E:\My\workspaces\C++_workspaces\ddtest\json)", json_paths, [](const std::wstring&, bool is_dir) {
        if (!is_dir) {
            return true;
        }
        return false;
    ...
    for (const auto& it : json_paths) {
        std::unique_ptr<ddfile> file(ddfile::create_utf8_file(it));
        if (file == nullptr) {
            ddcout(ddconsole_color::red) << ddstr::format(L"Open json file:%s failure.\r\n", it.c_str());
            continue;
        }

        std::string json_str(file->file_size(), 0);
        if ((s32)json_str.size() != file->read((u8*)json_str.data(), (s32)json_str.size())) {
            ddcout(ddconsole_color::red) << ddstr::format(L"Read json file:%s failure.\r\n", it.c_str());
            continue;
        }

        auto result = ddjson::parse(json_str);
        if (!result.successful) {
            ddcout(ddconsole_color::red) << ddstr::format(L"Parse json file:%s failure: %s.\r\n", it.c_str(), ddstr::utf8_16(result.error_message).c_str());
        } else {
            ddcout(ddconsole_color::green) << ddstr::format(L"Parse json file:%s successfully.\r\n", it.c_str());
        }
    }
}

DDTEST(ddjson_parse, ddjson_parse)
{
    std::string json_str = R"__([
  {
