...
public:
    ...
    ddcoroutine<std::pair<std::string, bool>> get_ip(const std::string& do_main, ddasync_caller async_caller);
    ddcoroutine<std::pair<std::string, bool>> get_ip(const ddurl& url, ddasync_caller async_caller);
...
private:
    ...
    // weak
    std::weak_ptr<dddns_factory> m_weak_this;
};
