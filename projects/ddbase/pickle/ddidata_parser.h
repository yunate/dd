...
{
    dddata_parse_state parse_state = dddata_parse_state::need_more_data;
    // 本次parse消费的字符数量
    s32 parsed_size = 0;

    inline void reset()
    {
        parse_state = dddata_parse_state::need_more_data;
        parsed_size = 0;
    }
};
...
public:
    // @return dddata_parse_state::complete 或者 dddata_parse_state::need_more_data
    // 获得以\n结尾的一行
