...
namespace NSP_DD {
template <class T>
const dddata_parse_result& ddhttp_data_parser<T>::continue_parse_from_buff(u8* buff, s32 buff_size)
{
...
template <class T>
void ddhttp_data_parser<T>::reset()
{
    m_context.reset();
...
template<class T>
const void* ddhttp_data_parser<T>::get_context()
{
    return &m_context;
}

template<class T>
const typename ddhttp_data_parser<T>::context* ddhttp_data_parser<T>::get_context_t()
{
    return (ddhttp_data_parser<T>::context*)get_context();
}

template<>
void ddhttp_data_parser<ddhttp_request_header>::parse_header_first_line_from_buff(u8* buff, s32 buff_size, dddata_parse_result& result)
{
    std::string line;
...
template<>
void ddhttp_data_parser<ddhttp_response_header>::parse_header_first_line_from_buff(u8* buff, s32 buff_size, dddata_parse_result& result)
{
    std::string line;
    ddidata_parser::get_rnline_from_buff(buff, buff_size, line, result);
    if (result.parse_state != dddata_parse_state::complete) {
...
template<class T>
void ddhttp_data_parser<T>::parse_header_kvs_from_buff(u8* buff, s32 buff_size, dddata_parse_result& result)
{
    std::string line;
    ...
    while (true) {
        ...
        m_context.header.kvs.set(key, value);
    }
}

template <class T>
void ddhttp_data_parser<T>::parse_body_size_from_buff(u8* buff, s32 buff_size, dddata_parse_result& result)
{
    m_body_recv_size = 0;
    m_chunked_size = 0;
...
template <class T>
void ddhttp_data_parser<T>::parse_body_from_buff(u8* buff, s32 buff_size, dddata_parse_result& result)
{
    if (m_chunked_size == 0) {
        if (!m_is_chunked) {
...
template <class T>
void ddhttp_data_parser<T>::parse_chunked_end_line_from_buff(u8* buff, s32 buff_size, dddata_parse_result& result)
{
...
template <class T>
void ddhttp_data_parser<T>::parse_trailer_from_buff(u8*, s32, dddata_parse_result& result)
{
    m_parse_step = ddhttp_parse_step::parse_end;
    result.parse_state = dddata_parse_state::complete;
}

template class ddhttp_data_parser<ddhttp_request_header>;
template class ddhttp_data_parser<ddhttp_response_header>;
} // namespace NSP_DD
