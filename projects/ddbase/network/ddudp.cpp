...
void ddudp_socket::send_to(void* buff, s32 buff_size, const ddaddr& addr, const std::function<void(bool successful, s32 byte)>& callback, ddexpire expire /* = ddexpire::never */)
{
    ddio_opt opt;
    opt.exec = [this, buff, buff_size, addr](ddiiocp_dispatch* dispatch, OVERLAPPED* ov) {
...
ddcoroutine<std::tuple<bool, s32>> ddudp_socket::send_to(void* buff, s32 buff_size, const ddaddr& addr, ddexpire expire /* = ddexpire::never */)
{
    ...
    co_await ddco_async([this, &return_value, buff, buff_size, &addr, expire](const ddresume_helper& resumer) {
        send_to(buff, buff_size, addr, [resumer, &return_value](bool successful, s32 byte) {
...
void ddudp_socket::recv_from(void* buff, s32 buff_size, ddaddr& addr, const std::function<void(bool successful, s32 byte)>& callback, ddexpire expire /* = ddexpire::never */)
{
#ifdef _DEBUG
    DDASSERT_FMTW(m_has_listened, L"the listen() function should be called before call recv_from() function.");
#endif
    ddio_opt opt;
    opt.exec = [this, buff, buff_size](ddiiocp_dispatch* dispatch, OVERLAPPED* ov) {
        DDASSERT(dispatch != nullptr);
        auto handle = dispatch->get_handle();
        if (handle == NULL) {
            dderror_code::set_last_error(dderror_code::init_failure);
    ...
    opt.is_end = nullptr;
    opt.callback = [callback, this, &addr](bool successful, s32 byte, bool) {
        if (successful) {
            addr.from_sockaddr((::sockaddr*)m_addr_buff);
        }
        callback(successful, byte);
    };
    ddiocp_io_dispatch_base::read(opt, expire);
}
...
ddcoroutine<std::tuple<bool, s32>> ddudp_socket::recv_from(void* buff, s32 buff_size, ddaddr& addr, ddexpire expire /* = ddexpire::never */)
{
    std::tuple<bool, s32> return_value;
    co_await ddco_async([this, &return_value, buff, buff_size, &addr, expire](const ddresume_helper& resumer) {
        recv_from(buff, buff_size, addr, [resumer, &return_value](bool successful, s32 byte) {
            std::get<0>(return_value) = successful;
            std::get<1>(return_value) = byte;
            resumer.lazy_resume();
        }, expire);
