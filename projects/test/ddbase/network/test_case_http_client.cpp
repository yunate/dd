...
ddcoroutine<std::string> get_ip(dddns_factory* factory, const ddurl& url)
{
    ...
    co_await ddco_async([&return_value, factory, url](const ddresume_helper& resumer) {
...
ddcoroutine<bool> send_header(ddhttp_client* http, ddtls* tls, const ddhttp_request_header& request_header, ddexpire expire)
{
    std::string header_str = request_header.to_str();
    auto buff_stream_view = ddstream_view::from((void*)header_str.c_str(), (s32)header_str.size());
    auto stream_view = ddstream_view::from(buff_stream_view.get(), tls);
    ddistream_view* view = stream_view.get();
    bool return_value = false;
    co_await NSP_DD::ddco_async([http, view, expire, &return_value](const NSP_DD::ddresume_helper& resumer) {
        http->send_stream(view, [resumer, &return_value](bool successful, s32, bool all_sended) {
...
ddcoroutine<void> co_http(ddiocp_with_dispatcher* iocp)
{
    ...
    auto http = ddhttp_client::create_inst(async_caller);
    DDASSERT(http != nullptr);
    if (!co_await http->connect(iocp, addr, expire)) {
        ddcout(ddconsole_color::red) << "connect failure: " << dderror_code::get_last_error_msga() << "\n";
        co_return;
    }

    if (use_tls) {
        if (!co_await http->https_hand_shake(url.host, expire)) {
            ddcout(ddconsole_color::red) << "https_hand_shake failure: " << dderror_code::get_last_error_msga() << "\n";
            co_return;
        }
    }
