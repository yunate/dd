...
static bool query_process_ids(std::vector<DWORD>& process_ids)
{
    process_ids.clear();
    ...
    ddexec_guard guard([&snapshot]() { (void)::CloseHandle(snapshot); });

    PROCESSENTRY32W process_entry = { sizeof(process_entry) };
    BOOL successful = ::Process32FirstW(snapshot, &process_entry);
    if (!successful) {
        DDLOG_LASTERROR();
        return false;
    }
    ...
    while (successful) {
        ...
        successful = ::Process32NextW(snapshot, &process_entry);
    }
    return true;
}
...
bool ddprocess::enum_mapped_files(
    const std::function<void(const ddhandle_info&)>& pred,
    s32 thread_count /* = 32 */
)
{
    ...
    if (!query_process_ids(process_ids)) {
        return false;
    }
    ...
    if ((size_t)thread_count > process_ids.size()) {
        thread_count = (s32)process_ids.size();
    }

    SYSTEM_INFO system_info = {};
    ::GetSystemInfo(&system_info);
    ULONG_PTR minimum_address = (ULONG_PTR)system_info.lpMinimumApplicationAddress;
    ULONG_PTR maximum_address = (ULONG_PTR)system_info.lpMaximumApplicationAddress;
    ...
    for (s32 i = 0; i < thread_count; ++i) {
        threads.emplace_back([&process_ids, &next_index, &pred_mutex, &pred, minimum_address, maximum_address]() {
            std::vector<WCHAR> path_buffer(32768);
            while (true) {
                size_t index = next_index.fetch_add(1);
                if (index >= process_ids.size()) {
                    break;
                }

                DWORD process_id = process_ids[index];
                ...
                std::unordered_set<ULONG_PTR> allocation_bases;
                ULONG_PTR address = minimum_address;
                while (address < maximum_address) {
                    ...
                    ULONG_PTR base_address = (ULONG_PTR)memory_info.BaseAddress;
                    ULONG_PTR next_address = base_address + (ULONG_PTR)memory_info.RegionSize;
                    if (next_address <= address || next_address < base_address) {
                        break;
                    }

                    if (memory_info.State == MEM_COMMIT &&
                        (memory_info.Type == MEM_IMAGE || memory_info.Type == MEM_MAPPED)) {
                        ULONG_PTR allocation_base = (ULONG_PTR)memory_info.AllocationBase;
                        if (allocation_base != 0 && allocation_bases.insert(allocation_base).second) {
                            DWORD path_length = ::K32GetMappedFileNameW(
                                process,
                                memory_info.AllocationBase,
                                path_buffer.data(),
                                (DWORD)path_buffer.size()
                            );
                            if (path_length > 0 && path_length < path_buffer.size()) {
                                ddhandle_info info;
                                info.process_id = process_id;
                                info.type_name = memory_info.Type == MEM_IMAGE ? L"Image" : L"MappedFile";
                                info.base_object_name = nt_path_to_dos_path(
                                    std::wstring(path_buffer.data(), path_length)
                                );
...
bool ddprocess::enum_file_usages(
    const std::function<void(const ddhandle_info&)>& pred,
    s32 thread_count /* = 32 */
)
{
    ...
    bool handles_successful = enum_file_handles(pred, thread_count);
    bool mappings_successful = enum_mapped_files(pred, thread_count);
    return handles_successful && mappings_successful;
}
