...
void searcher::search_file(const std::wstring& file_path)
{
    ...
    while (true) {
        ...
        if (!push_result) {
            (void)m_pool->wait_queue_not_full();
        } else {
            break;
        }
    }
...
void searcher::search_dir(const std::wstring& dir_path)
{
    ...
    while (!dirs.empty()) {
        ...
        dirs.pop();
        dddir::enum_dir_first_level(path, [this, &dirs, &path](const std::wstring& name, bool is_dir) {
            if (name == L"" || name == L"out" || name[0] == '.' || name == L"third_party") {
                return false;
            }
            std::wstring full_path = ddpath::join(path, name);
            if (is_dir) {
                dirs.push(full_path);
            } else {
