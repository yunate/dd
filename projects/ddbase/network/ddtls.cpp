...
void ddtls::set_aync_caller(const ddasync_caller& async_caller)
{
...
ddcoroutine<bool> ddtls::handshake(const pfn_io_opt& read_opt, const pfn_io_opt& write_opt, ddexpire expire /* = ddexpire::never */)
{
    ...
    while (true) {
        ...
        if (!m_async_caller) {
            ...
            co_await ddawaitable([this, &handshake_inner_result, weak = m_weak_this](const ddresume_helper& resumer) {
                m_async_caller([&, weak, resumer]() {
                    auto sp = weak.lock();
                    if (sp == nullptr) {
                        return;
                    }
                    // run async, maybe in another thread.
                    handshake_inner_result = handshake_inner(m_handshake_ctx.in_buff.data(), m_handshake_ctx.in_buff_size,
                        m_handshake_ctx.out_buff.data(), m_handshake_ctx.out_buff_size);
                    resumer.lazy_resume();
                });
...
ddcoroutine<ddtls::codec_result> ddtls::encode(u8* buff, s32 buff_size)
{
    ...
    if (!m_async_caller) {
        ...
        co_await ddawaitable([&, weak = m_weak_this](const ddresume_helper& resumer) {
            m_async_caller([&, weak, resumer]() {
                auto sp = weak.lock();
                if (sp == nullptr) {
                    return;
...
ddcoroutine<ddtls::codec_result> ddtls::decode(u8* buff, s32 buff_size)
{
...
namespace NSP_DD {
ddtls_server::ddtls_server(const std::shared_ptr<ddcert>& cert) : m_cert(cert)
{
    m_handshake_ctx.complete = false;
    m_handshake_ctx.need_recv = true;
    m_handshake_ctx.need_send = false;
...
bool ddtls_server::handshake_inner(u8* in_buff, s32& in_buff_size, u8* out_buff, s32& out_buff_size)
{
    DDASSERT_FMT(!m_handshake_ctx.complete, L"the handshake have handshake successful, do not need handshake again");
    SECURITY_STATUS ss;
