...
static ddcoroutine<void> co_http(ddiocp_with_dispatcher* iocp)
{
    ...
    auto requester = ddhttp_requester::make_get_requester(iocp, dns.get(), async_caller, url_str);
    if (requester == nullptr) {
        ddcout(ddconsole_color::red) << "make_get_requester failure: " << dderror_code::get_last_error_msga() << "\n";
        co_return;
    }

    const ddhttp_response_header* header = co_await requester->recv_header(expire);
    if (header == nullptr) {
