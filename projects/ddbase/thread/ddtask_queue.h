...
public:
    ...
    ddtask_queue(u32 max_count = MAX_U32, const std::function<void()>& on_any_task_complete = nullptr);
    ~ddtask_queue();
    void set_max_count(u32 max_count);
    void set_any_task_complete_handler(const std::function<void()>& on_any_task_complete);
    u32 get_max_count();
    u64 get_task_count();
...
private:
    ...
    std::function<void()> m_on_any_task_complete;
};
